...
- необязательный публичный медиа-дашборд для MP3 и Ogg SFX с приватными серверными лимитами владельца;
- отдельная серверная SFX-библиотека; прозрачный путь скачивания SFX оригинальным клиентом ещё проходит отдельную проверку;
...
`/mediaAdmin.php` — необязательная публичная библиотека/загрузчик. MP3-песни и Ogg SFX можно загружать без admin token, когда владелец включает `public_uploads` в приватном, исключённом из Git файле `config/media.php`.

Посетители видят текущие лимиты, но не могут их менять или удалять медиа. Владелец задаёт `song_max_mib` и `sfx_max_mib` в `config/media.php`; лимиты PHP/Nginx/хостинга по-прежнему остаются верхней границей.

Настройка, права файла, безопасность публичных загрузок и текущая граница совместимости SFX: `docs/ru/MEDIA_DASHBOARD.md`.
...
- собственную серверную MP3-библиотеку с загрузкой через публичную медиа-страницу, когда она включена, либо через старый защищённый токеном `/songAdmin.php` для владельца.

Новым локальным трекам выдаются Song ID из настраиваемого семизначного диапазона `2000000..8999999` по умолчанию. Старые песни с высокими ID продолжают работать и автоматически не перенумеровываются.
...
Night Core V1 уже проходит проверку реальным Geometry Dash 2.2 клиентом: регистрация, вход, профиль, уровни, комментарии, лайки, social-функции и серверные кастомные песни проверяются непосредственно через GDPS.
