...
- собственная серверная MP3-библиотека с автоматической выдачей Song ID;
...
Night Core поддерживает два независимых варианта:

- внешний Newgrounds/Boomlings resolver для неизвестных ID, когда upstream доступен;
- собственную серверную MP3-библиотеку с загрузкой через защищённый токеном `/songAdmin.php`.

Локально загруженным трекам автоматически выдаются Song ID из диапазона `90000000..99999999`, а MP3 раздаёт сам Night Core. Поэтому такие песни не зависят от доступности внешних аудиосервисов.

Инструкция по собственной библиотеке: `docs/ru/CUSTOM_SONGS.md`. Настройка внешнего resolver: `docs/ru/NEWGROUNDS.md`.
