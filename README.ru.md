...
- закрытый медиа-дашборд для песен, Ogg SFX и runtime-лимитов загрузки;
...
`/mediaAdmin.php` — основной интерфейс владельца для локального аудио. Через него можно загружать/удалять MP3-песни и Ogg SFX, видеть локальные библиотеки и менять максимальный размер одного файла без редактирования `.env`.

Изменённые лимиты сохраняются в MariaDB. Например, лимит песни можно сразу уменьшить с 25 MiB до 10 MiB. Лимиты PHP/Nginx/хостинга при этом остаются верхней границей инфраструктуры.

Настройка и текущая граница совместимости SFX с оригинальным клиентом: `docs/ru/MEDIA_DASHBOARD.md`.
...
- собственную серверную MP3-библиотеку с загрузкой через `/mediaAdmin.php` или старый защищённый токеном `/songAdmin.php`.
