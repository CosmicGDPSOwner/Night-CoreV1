...
2. Night Core выдаёт Song ID из настраиваемого локального диапазона. По умолчанию используется `2000000..8999999`, то есть семизначные ID вместо прежнего диапазона `90000000+`.
...
Уже загруженные локальные песни со старыми высокими ID остаются рабочими и автоматически не перенумеровываются. Новые загрузки используют настроенный низкий диапазон.
...
`CUSTOM_SONG_ID_MIN` и `CUSTOM_SONG_ID_MAX` задают диапазон ID, зарезервированный для локальных песен. Значения по умолчанию — `2000000` и `8999999`. Night Core теперь выделяет ID явно внутри этого диапазона, поэтому старые записи с высокими ID не заставляют новые загрузки снова уходить в диапазон `90000000+`. Для другого GDPS диапазон можно изменить, но он не должен пересекаться с внешним каталогом песен, который владелец намеренно импортирует.
