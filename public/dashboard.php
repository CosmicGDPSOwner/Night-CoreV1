...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    try {
        ...
        } elseif ($action === 'save_sensitive_confirmation') {
            if ($loggedAccountID <= 0 || $loggedAccount === null) {
                throw new RuntimeException('Sign in before changing security settings.');
            }
            $securityStatus = $confirmation->save(
                $loggedAccountID,
                (string) ($_POST['current_password'] ?? ''),
                isset($_POST['require_sensitive_password'])
            );
            $message = $securityStatus['requirePassword']
                ? 'Password confirmation enabled for sensitive actions.'
                : 'Password confirmation disabled for sensitive actions.';
            $authPanelOpen = true;
...
if ($loggedAccountID > 0 && $loggedAccount !== null) {
    ...
    if ($securityStatus === null) {
        try {
            $securityStatus = $confirmation->status($loggedAccountID);
        } catch (Throwable) {
            $securityStatus = null;
        }
    }
}
$requireSensitivePassword = $loggedAccountID <= 0
    || $securityStatus === null
    || (bool) $securityStatus['requirePassword'];
