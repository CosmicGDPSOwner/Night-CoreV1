...
$publicBaseUrl = static function (string $configKey, string $fallbackKey = ''): string {
    ...
    if ($baseUrl === '' && $fallbackKey !== '') $baseUrl = trim(Config::get($fallbackKey, '') ?? '');
...
$startAccountSession = static function (array $account) use (&$loggedAccountID, &$loggedAccount, $fingerprint): void {
    session_regenerate_id(true);
    ...
    try {
        ...
        if ($action === 'login') {
            ...
            if (!$app->schema()->tableExists('core_registration_attempts')) {
                throw new RuntimeException('Apply migration 0020 before registering through the dashboard.');
            }
            ...
            if (!$publicUploads) throw new RuntimeException('Media uploads are disabled.');
            ...
            if ($action === 'upload_song') {
                if (!isset($_FILES['song']) || !is_array($_FILES['song'])) throw new RuntimeException('Choose an MP3 file.');
                $upload = $_FILES['song'];
                ...
                if ($tmpName === '' || !is_uploaded_file($tmpName)) throw new RuntimeException('The uploaded MP3 was not received as a valid HTTP upload.');
                $bytes = filesize($tmpName);
                if ($bytes === false || $bytes <= 0) throw new RuntimeException('The uploaded MP3 is empty.');
                ...
                if (!isset($_FILES['sfx']) || !is_array($_FILES['sfx'])) throw new RuntimeException('Choose an OGG file.');
                $upload = $_FILES['sfx'];
                $uploadError($upload, 'OGG');
                $tmpName = isset($upload['tmp_name']) && is_string($upload['tmp_name']) ? $upload['tmp_name'] : '';
                $originalName = isset($upload['name']) && is_string($upload['name']) ? $upload['name'] : 'sfx.ogg';
                if ($tmpName === '' || !is_uploaded_file($tmpName)) throw new RuntimeException('The uploaded OGG was not received as a valid HTTP upload.');
                $bytes = filesize($tmpName);
                if ($bytes === false || $bytes <= 0) throw new RuntimeException('The uploaded OGG is empty.');
        ...
        $error = $e->getMessage();
        if ($action === 'login' || $action === 'register') $authPanelOpen = true;
    }
}
...
<div class="dialog-header"><div><span class="pill">GDPS account</span><h2><?= $loggedAccount !== null ? 'Your account' : 'Account access' ?></h2></div><button type="button" class="close-button" data-close-account aria-label="Close">×</button></div>
...
<p class="muted">Use the same username and password as in Geometry Dash. Passwords are verified and never stored in the browser session.</p>
<form method="post">
...
<p class="muted">Creates a normal GDPS account through the same registration service, password hashing, activation policy and anti-abuse limits used by the game endpoint.</p>
<form method="post">
...
<label>Password</label><input type="password" name="password" autocomplete="new-password" required>
<label>Repeat password</label><input type="password" name="password_confirm" autocomplete="new-password" required>
<small>Registration attempts are rate-limited by hashed IP and subnet identifiers. Raw IP addresses are not stored.</small><br><button type="submit">Create account</button>
</form>
</section>
<?php endif; ?>
</div>
