    ...
    try {
        if ($action === 'login') {
            ...
            if ($action === 'save_role') {
                ...
                $modBadgeLevel = max(0, min(2, (int) ($_POST['mod_badge_level'] ?? 0)));
                $badgeText = $field($_POST['badge_text'] ?? '', 24);
                $badgeColor = $color($_POST['badge_color'] ?? '');
                $commentColor = $color($_POST['comment_color'] ?? '');
                $usernameColor = $color($_POST['username_color'] ?? '');
                $permissions = isset($_POST['permissions']) && is_array($_POST['permissions']) ? $_POST['permissions'] : [];
                $savedID = $repository->saveRole($roleID, $name, $priority, $modBadgeLevel, $badgeText, $badgeColor, $commentColor, $usernameColor, $permissions);
                $message = 'Role saved. ID: ' . $savedID . '.';
            } elseif ($action === 'delete_role') {
                $roleID = (int) ($_POST['role_id'] ?? 0);
                if ($roleID <= 0 || !$repository->deleteRole($roleID)) {
                ...
                if ($staff->isOwner($targetAccountID)) {
                    throw new RuntimeException('Owner accounts do not need a staff role.');
                }
...
$csrfValue = $csrf();
$roles = $loggedAccountID > 0 ? $repository->roles() : [];
